...
public:
  ...
  void writeToMathematicaFile(const char* aFilename) const;
  ...
  void writeToPPM(const char* aFilename) const;
  // Reads the tensor from a PDM file
  void readFromPDM(const char* aFilename);
  // Writes the tensor in PDM-Format
  void writeToPDM(const char* aFilename, char aFeatureType) const;

#ifdef HAVE_PNG
  // Reads a RGB(A) png file. The zSize for RGBA images is 4 
  void readFromPNG(const char* aFilename, bool ignoreAlpha=false);
  // Writes a RGB(A) png file.
  void writeToPNG(const char* aFilename) const;
#endif

  // Reads a tensor from an image file. Uses the suffix to determine the format
  void readFromImage(const char* aFilename);
...
template <class T>
void CTensor<T>::downsample(int aNewXSize, int aNewYSize, int aNewZSize)
{
  downsample(aNewXSize, aNewYSize);
  T* mData2 = new T[aNewXSize*aNewYSize*aNewZSize];
  // we downsample XZ slices
#ifdef _OPENMP
#pragma omp parallel for
#endif
  for( int y = 0; y < mYSize; ++y )
  {
    CMatrix<T> aTemp(mXSize,mZSize);
    for( int z = 0; z < mZSize; ++z )
    for( int x = 0; x < mXSize; ++x )
    {
      aTemp(x,z) = operator()(x,y,z);
    }
    aTemp.downsample(mXSize, aNewZSize);

    int sizeNewXY = aNewXSize*aNewYSize;

    for( int z = 0; z < aNewZSize; ++z )
    for( int x = 0; x < mXSize; ++x )
    {
      mData2[sizeNewXY*z + aNewXSize*y + x] = aTemp(x,z);
    }
  }
  delete[] mData;
  mData = mData2;
...
void CTensor<T>::rgbToCielab() {
  for (int y = 0; y < mYSize; y++)
    for (int x = 0; x < mXSize; x++) {
      ...
      X *= 1.052111;
      Z *= 0.918417;
      if (X > 0.008856) X = powf(X,0.33333333333); else X = 7.787*X + 0.137931034;
      if (Y > 0.008856) Y = powf(Y,0.33333333333); else Y = 7.787*Y + 0.137931034;
      if (Z > 0.008856) Z = powf(Z,0.33333333333); else Z = 7.787*Z + 0.137931034;
...
void CTensor<T>::cielabToRGB() {
  for (int y = 0; y < mYSize; y++)
    for (int x = 0; x < mXSize; x++) {
      ...
      if (r < 0) r = 0;
      temp = 1.055*powf(r,0.41667)-0.055;
      if (temp > 0.0031308) r = temp;
      else r *= 12.92;
      if (g < 0) g = 0;
      temp = 1.055*powf(g,0.41667)-0.055;
      if (temp > 0.0031308) g = temp;
      else g *= 12.92;
      if (b < 0) b = 0;
      temp = 1.055*powf(b,0.41667)-0.055;
...
template <class T>
void CTensor<T>::writeToMathematicaFile(const char* aFilename) const{
  using namespace std;
...
// writeToIMFile
template <class T>
void CTensor<T>::writeToIMFile(const char *aFilename) const{
  FILE *aStream;
...
// writeToPGM
template <class T>
void CTensor<T>::writeToPGM(const char* aFilename) const {
...
// writeToPPM
template <class T>
void CTensor<T>::writeToPPM(const char* aFilename) const{
  FILE* outimage = fopen(aFilename, "wb");
  fprintf(outimage, "P6 \n");
...
// writeToPDM
template <class T>
void CTensor<T>::writeToPDM(const char* aFilename, char aFeatureType) const {
...
#ifdef HAVE_PNG
template <class T>
void CTensor<T>::readFromPNG( const char* aFilename, bool ignoreAlpha )
{
  FILE *file = fopen(aFilename, "rb");
  if( !file )
  {
    std::string err( "CTensor::readFromPNG: Cannot open file " );
    err += aFilename;
  ...
  // read the header
  png_byte header[8];
  size_t count = fread(header, 1, 8, file);
  if( count < 8 )
  {
    fclose(file);
    std::string err( "CTensor::readFromPNG: Could not read header of file " );
    err += std::string(aFilename);
    throw std::runtime_error(err);
  }
  if( png_sig_cmp(header, 0, 8) )
  {
    fclose(file);
    std::string err( "CTensor::readFromPNG: File " );
    err += std::string(aFilename) + std::string(" is not a PNG");
    throw std::runtime_error(err);
  }
  ...
  {
    fclose(file);
    throw std::runtime_error( "CTensor::readFromPNG: Could not create png read struct" );
  }
  ...
  if( !info_ptr )
  {
    ...
    fclose(file);
    throw std::runtime_error( "CTensor::readFromPNG: Could not create png info struct" );
  }

  // png_init_io will call longjmp on errors
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    png_destroy_info_struct(png_ptr, &info_ptr);
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error( "CTensor::readFromPNG: png_init_io failed" );
  }

  png_init_io(png_ptr, file);

  // tells libpng that we already have read the header
      ...
      || bit_depth != 8 )
  {
    ...
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error("CTensor::readFromPNG: Format not supported. "
                             " Expected RGB(A) image with 8-bit per channel");
  }
  ...
  // jump for errors during read process
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    png_destroy_info_struct(png_ptr, &info_ptr);
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error( "CTensor::readFromPNG: png_read_image failed" );
  }
  ...
  mXSize = width;
  mYSize = height;
  mZSize = ignoreAlpha ? 3 : pixel_size;
  mData = new T[mXSize*mYSize*mZSize];
  ...
  for( int y = 0; y < height; ++y )
  {
    ...
    {
      png_byte* pix = &row[channels*x];
      for( int c = 0; c < mZSize; ++c )
        operator()(x,y,c) = (T) pix[c];
    }
  }
...
template <class T>
void CTensor<T>::writeToPNG( const char* aFilename ) const
{
  if( mZSize < 3 || mZSize > 4 )
    throw std::runtime_error( "CTensor::writeToPNG: zSize must be 3 or 4" );
    
  FILE *file = fopen(aFilename, "wb");
  if( !file )
  {
    std::string err( "CTensor::writeToPNG: Cannot open file " );
    err += aFilename;
    throw std::runtime_error(err);
  }
  ...
  {
    fclose(file);
    throw std::runtime_error( "CTensor::writeToPNG: Could not create png write struct" );
  }
  ...
  if( !info_ptr )
  {
    ...
    fclose(file);
    throw std::runtime_error( "CTensor::writeToPNG: Could not create png info struct" );
  }
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CTensor::writeToPNG: png_init_io failed" );
  }
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CTensor::writeToPNG: png_write_info failed" );
  }

  png_set_IHDR( png_ptr, info_ptr, mXSize, mYSize, 8, 
                (mZSize==4) ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB,
                PNG_INTERLACE_NONE,
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CTensor::writeToPNG: png_write_image failed" );
  }

  // convert image channels to 8-bit
  png_bytep* row_pointers = (png_bytep*) malloc(sizeof(png_bytep) * mYSize);
  for( int y = 0; y < mYSize; y++ )
    row_pointers[y] = (png_byte*) malloc(sizeof(png_byte)*mZSize*mXSize);
  ...
  for( int y = 0; y < mYSize; y++ )
  {
    ...
    for( int x = 0; x < mXSize; x++ )
    {
      png_bytep pixel = row_ptr + x*mZSize;
      for( int c = 0; c < mZSize; c++ )
        pixel[c] =  (unsigned char) operator()(x,y,c);
    }
  }
...
template <class T>
void CTensor<T>::readFromImage(const char* aFilename)
{
  std::string filename(aFilename);
  size_t last_dot = filename.find_last_of('.');
  std::string suffix( filename.substr(last_dot+1) );

  if( suffix == "ppm" || suffix == "PPM" )
  {
    readFromPPM(aFilename);
  }
  ...
  {
    readFromPNG(aFilename);
  }
#endif
  else
  {
    throw std::runtime_error("Image file format not supported");
  }
}
...
CTensor<T>& CTensor<T>::operator=(const CTensor<T>& aCopyFrom) {
  if (this != &aCopyFrom)
  {
    if (aCopyFrom.mData == 0)
    {
      ...
      // avoid memory reallocation if the size is compatible
      if( size() == aCopyFrom.size() )
      {
        mXSize = aCopyFrom.mXSize;
        mYSize = aCopyFrom.mYSize;
        mZSize = aCopyFrom.mZSize;
        size_t copy_size =  sizeof(T)*size();
        memcpy( (void*) mData, (void*) aCopyFrom.mData, copy_size );
      }
      else
      {
        delete[] mData;
