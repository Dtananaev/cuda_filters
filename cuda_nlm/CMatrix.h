...
//  void upsampleBicubic(int aNewXSize, int aNewYSize);
  ...
  void writeToTXT(const char* aFilename, bool aHeader = true) const;
  // Reads a projection matrix in a format used by Bodo Rosenhahn
  void readBodoProjectionMatrix(const char* aFilename);

#ifdef HAVE_PNG
  // Reads a Grayscale png file. 
  void readFromPNG(const char* aFilename);
  // Writes a Grayscale png file.
  // If T == unsigned short then a 16-bit PNG grayscale file is written.
  // For all other types values are casted to unsigned char.
...
// writeToPGM
template <class T>
void CMatrix<T>::writeToPGM(const char *aFilename) const {
  FILE *aStream;
...
//writeToTXT
template <class T>
void CMatrix<T>::writeToTXT(const char* aFilename, bool aHeader) const {
...
#ifdef HAVE_PNG
template <class T>
void CMatrix<T>::readFromPNG( const char* aFilename )
{
  FILE *file = fopen(aFilename, "rb");
  if( !file )
  {
    std::string err( "CMatrix::readFromPNG: Cannot open file " );
    err += aFilename;
  ...
  {
    fclose(file);
    std::string err( "CMatrix::readFromPNG: Could not read header of file " );
  ...
  {
    fclose(file);
    std::string err( "CMatrix::readFromPNG: File " );
  ...
  {
    fclose(file);
    throw std::runtime_error( "CMatrix::readFromPNG: Could not create png read struct" );
  }
  ...
  if( !info_ptr )
  {
    ...
    fclose(file);
    throw std::runtime_error( "CMatrix::readFromPNG: Could not create png info struct" );
  }
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error( "CMatrix::readFromPNG: png_init_io failed" );
  }
  ...
  if( color_type != PNG_COLOR_TYPE_GRAY || (bit_depth != 8 && bit_depth != 16) )
  {
    ...
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error("CMatrix::readFromPNG: Format not supported. "
                             " Expected Greyscale image with 8 or 16 bit per channel");
  }

  //int number_of_passes = png_set_interlace_handling(png_ptr);
  png_set_interlace_handling(png_ptr);
  if( bit_depth == 16 ) // png stores data in big endian order -> enable swap to little endian
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_read_struct(&png_ptr, 0, 0);
    fclose(file);
    throw std::runtime_error( "CMatrix::readFromPNG: png_read_image failed" );
  }
  ...
  if( bit_depth == 16 )
  {
    for( int y = 0; y < height; ++y )
    {
      ...
      {
        unsigned short* pix = (unsigned short*)row;
        pix += x;
        operator()(x,y) = (T) *pix;
      }
    }
...
template <>
inline 
void CMatrix<unsigned short>::writeToPNG( const char* aFilename ) const
{
  FILE *file = fopen(aFilename, "wb");
  if( !file )
  {
    std::string err( "CMatrix::writeToPNG: Cannot open file " );
    err += aFilename;
    throw std::runtime_error(err);
  }
  ...
  {
    fclose(file);
    throw std::runtime_error( "CMatrix::writeToPNG: Could not create png write struct" );
  }
  ...
  if( !info_ptr )
  {
    ...
    fclose(file);
    throw std::runtime_error( "CMatrix::writeToPNG: Could not create png info struct" );
  }
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CMatrix::writeToPNG: png_init_io failed" );
  }
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CMatrix::writeToPNG: png_write_info failed" );
  }

  png_set_IHDR( png_ptr, info_ptr, mXSize, mYSize, 16, 
                PNG_COLOR_TYPE_GRAY,
                PNG_INTERLACE_NONE,
  ...
  png_set_swap(png_ptr); // data in memory is little endian but png stores data in big endian order
  ...
  if( setjmp(png_jmpbuf(png_ptr)) )
  {
    ...
    png_destroy_write_struct(&png_ptr, 0);
    fclose(file);
    throw std::runtime_error( "CMatrix::writeToPNG: png_write_image failed" );
  }

  // convert image channels to 16-bit
  png_bytep* row_pointers = (png_bytep*) malloc(sizeof(png_bytep) * mYSize);
  for( int y = 0; y < mYSize; y++ )
    row_pointers[y] = (png_byte*) malloc(sizeof(unsigned short)*mXSize);
  ...
  for( int y = 0; y < mYSize; y++ )
  {
    ...
    for( int x = 0; x < mXSize; x++ )
    {
      unsigned short* pixel = (unsigned short*)(row_ptr + 2*x);
      *pixel =  operator()(x,y);
    }
  }
...
template <class T>
void CMatrix<T>::writeToPNG( const char* aFilename ) const
{
  FILE *file = fopen(aFilename, "wb");
  if( !file )
  {
  ...
  png_bytep* row_pointers = (png_bytep*) malloc(sizeof(png_bytep) * mYSize);
  for( int y = 0; y < mYSize; y++ )
    row_pointers[y] = (png_byte*) malloc(sizeof(png_byte)*mXSize);
  ...
  for( int y = 0; y < mYSize; y++ )
  {
    ...
    for( int x = 0; x < mXSize; x++ )
    {
      png_bytep pixel = row_ptr + x;
      *pixel =  (unsigned char) operator()(x,y);
    }
  }
